...
program
  ...
  .action(async (name) => {
    try {
      ...
      await updateDocumentation(name);
  ...
  .action((query) => {
    try {
      ...
      if (results.documentationMatches.length > 0) {
        console.log(chalk.green(`\nFound ${results.documentationMatches.length} documentation sources:`));
        
        results.documentationMatches.forEach((result, index) => {
          console.log(chalk.cyan(`\n${index + 1}. ${result.source.name} (${result.source.url})`));
          
          if (result.matches.headings.length > 0) {
            console.log(chalk.yellow('  Matching headings:'));
            result.matches.headings.forEach(heading => {
              console.log(`  - ${heading.text}`);
            });
          }
          
          if (result.matches.codeBlocks.length > 0) {
            console.log(chalk.yellow('  Matching code blocks:'));
            result.matches.codeBlocks.forEach(block => {
